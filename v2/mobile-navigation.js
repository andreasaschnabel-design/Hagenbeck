...
function writeState(state) {
  try {
    ...
    // Navigation funktioniert auch ohne dauerhafte Speicherung.
  }
}

function toMapPosition(position) {
  const x = ((position.coords.longitude - GEO.OSM_BOUNDS.west) /
    (GEO.OSM_BOUNDS.ost - GEO.OSM_BOUNDS.west)) * 100;
  const y = ((GEO.OSM_BOUNDS.nord - position.coords.latitude) /
    (GEO.OSM_BOUNDS.nord - GEO.OSM_BOUNDS.sued)) * GEO.MASS.h;
  return { x, y };
}
...
function showMessage(text, isError = false) {
  ...
  showMessage.timer = setTimeout(() => banner.classList.remove('show'), 3200);
}
...
function storeNavigation(position, id) {
  ...
  state.view = 'map';
  writeState(state);
  sessionStorage.setItem('hagenbeck-v24-navigation-start', JSON.stringify(mapPosition));
  sessionStorage.setItem('hagenbeck-v24-navigation-target', id);
}
...
function startWatching() {
  ...
  watchId = navigator.geolocation.watchPosition(
    position => {
      ...
      writeState(state);
      sessionStorage.setItem('hagenbeck-v24-navigation-start', JSON.stringify(mapPosition));
      window.dispatchEvent(new CustomEvent('hagenbeck:navigation-position', { detail: mapPosition }));
    },
    () => {},
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    error => {
      const text = error.code === 1
        ? 'Standortfreigabe fehlt. Bitte in den Browser-Einstellungen erlauben.'
...
function interceptNavigate(event) {
...
window.addEventListener('load', () => {
  targetId = sessionStorage.getItem('hagenbeck-v24-navigation-target');
  if (targetId) startWatching();
});

const style = document.createElement('style');
style.textContent = `
  .mobile-navigation-message {
    position: fixed;
    z-index: 200;
    left: 14px;
    right: 14px;
  ...
  .mobile-navigation-message.show {
    opacity: 1;
    transform: translateY(0);
  }
  .mobile-navigation-message.error {
    background: rgba(88,24,20,.97);
  }
`;
